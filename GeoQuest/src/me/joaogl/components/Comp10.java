...
import me.joaogl.game.resolution.Resolution10;
...
public class Comp10 {
	...
	public static void drawCredits() {
		...
		int size = 560;
		lversion.setBounds(((Questions10.width - size) / 2), (Questions10.height - 80), size, 15);
		Questions10.window.add(lversion);
	}

	public static void counterDisplay() {
		new Thread(new Runnable() {
			public void run() {
				while (Questions10.running == true) {
					try {
						Thread.sleep(1000);
					...
					if (!(TotalTimer.getTime() == null)) {
						if (TotalTimer.getTime() == "15:0" || TotalTimer.getTime() == "O tempo acabou.") Questions10.counter.setText("Acabou!");
						else Questions10.counter.setText("Tempo: " + TotalTimer.getTime());
					}
					if (!(QuestionTimer.getTime() == 0)) {
						if (QuestionTimer.getTime() == QuestionTimer.sec) Questions10.countersec.setText("Acabou!");
						else Questions10.countersec.setText("Tempo: " + QuestionTimer.getTime());
					}
				}
			}
		}).start();
	...
	public static void drawMenu() {
		...
		Questions10.bar.add(About);
		Questions10.bar.add(Points);
		if (ProgramInfo.debugging) Questions10.bar.add(Debugging);
	...
	public static void drawTeams() {
		...
		size = 170;
		welcome.setBounds((Questions10.width - size) / 8, 200, size, 50);
		Questions10.window.add(welcome);

		welcome = new JLabel("Equipa 2 - " + Points.getBlue() + " Pontos");
		welcome.setFont(new Font("Arial", Font.BOLD, 18));
		welcome.setForeground(Color.BLUE);
		welcome.setBounds(((Questions10.width - size) * 7) / 8, 200, size, 50);
		Questions10.window.add(welcome);
	...
	public static void drawTimers() {
		...
		size = 210;
		tempo.setBounds(((Questions10.width - size) / 2) - 250, Questions10.height - 140, size, 50);
		Questions10.window.add(tempo);

		Questions10.counter = new JLabel("Tempo: 0");
		Questions10.counter.setFont(new Font("Arial", Font.BOLD, 15));
		size = 100;
		Questions10.counter.setBounds(((Questions10.width - size) / 2) - 250, Questions10.height - 120, size, 50);
		Questions10.window.add(Questions10.counter);

		tempo = new JLabel("Tempo Restante:");
		tempo.setFont(new Font("Arial", Font.BOLD, 15));
		size = 130;
		tempo.setBounds((Questions10.width - size) / 2, 120, size, 12);
		Questions10.window.add(tempo);

		Questions10.countersec = new JLabel("Tempo: " + (QuestionTimer.sec - 1));
		Questions10.countersec.setFont(new Font("Arial", Font.BOLD, 15));
		size = 80;
		Questions10.countersec.setBounds((Questions10.width - size) / 2, 140, size, 12);
		Questions10.window.add(Questions10.countersec);

	}

	public static void close() {
		Questions10.window.removeAll();
		if (ProgramInfo.debugging) {
			if (ProgramInfo.debugquestions == Questions10.pageid) new Questions10();
			else new Resolution10();
		} else new Resolution10();
	}
}