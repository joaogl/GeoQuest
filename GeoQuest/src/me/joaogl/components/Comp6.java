...
import me.joaogl.game.resolution.Resolution6;
...
public class Comp6 {
	...
	public static void drawCredits() {
		...
		int size = 560;
		lversion.setBounds(((Questions6.width - size) / 2), (Questions6.height - 80), size, 15);
		Questions6.window.add(lversion);
	}

	public static void counterDisplay() {
		new Thread(new Runnable() {
			public void run() {
				while (Questions6.running == true) {
					try {
						Thread.sleep(1000);
					...
					if (!(TotalTimer.getTime() == null)) {
						if (TotalTimer.getTime() == "15:0" || TotalTimer.getTime() == "O tempo acabou.") Questions6.counter.setText("Acabou!");
						else Questions6.counter.setText("Tempo: " + TotalTimer.getTime());
					}
					if (!(QuestionTimer.getTime() == 0)) {
						if (QuestionTimer.getTime() == QuestionTimer.sec) {
							Questions6.countersec.setText("Acabou!");
						} else {
							Questions6.countersec.setText("Tempo: " + QuestionTimer.getTime());
						}
					}
				}
			}
	...
	public static void drawMenu() {
		...
		Questions6.bar.add(About);
		Questions6.bar.add(Points);
		if (ProgramInfo.debugging) Questions6.bar.add(Debugging);
	...
	public static void drawTeams() {
		...
		size = 170;
		welcome.setBounds((Questions6.width - size) / 8, 200, size, 50);
		Questions6.window.add(welcome);

		welcome = new JLabel("Equipa 2 - " + Points.getBlue() + " Pontos");
		welcome.setFont(new Font("Arial", Font.BOLD, 18));
		welcome.setForeground(Color.BLUE);
		welcome.setBounds(((Questions6.width - size) * 7) / 8, 200, size, 50);
		Questions6.window.add(welcome);
	...
	public static void drawTimers() {
		...
		size = 210;
		tempo.setBounds(((Questions6.width - size) / 2) - 250, Questions6.height - 140, size, 50);
		Questions6.window.add(tempo);

		Questions6.counter = new JLabel("Tempo: 0");
		Questions6.counter.setFont(new Font("Arial", Font.BOLD, 15));
		size = 100;
		Questions6.counter.setBounds(((Questions6.width - size) / 2) - 250, Questions6.height - 120, size, 50);
		Questions6.window.add(Questions6.counter);

		tempo = new JLabel("Tempo Restante:");
		tempo.setFont(new Font("Arial", Font.BOLD, 15));
		size = 130;
		tempo.setBounds((Questions6.width - size) / 2, 120, size, 12);
		Questions6.window.add(tempo);

		Questions6.countersec = new JLabel("Tempo: " + (QuestionTimer.sec - 1));
		Questions6.countersec.setFont(new Font("Arial", Font.BOLD, 15));
		size = 80;
		Questions6.countersec.setBounds((Questions6.width - size) / 2, 140, size, 12);
		Questions6.window.add(Questions6.countersec);

	}

	public static void close() {
		Questions6.window.removeAll();
		if (ProgramInfo.debugging) {
			if (ProgramInfo.debugquestions == Questions6.pageid) new Questions6();
			else new Resolution6();
		} else new Resolution6();
	}
}