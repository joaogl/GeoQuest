...
import me.joaogl.game.resolution.Resolution3;
...
public class Comp3 {
	...
	public static void drawCredits() {
		...
		int size = 560;
		lversion.setBounds(((Questions3.width - size) / 2), (Questions3.height - 80), size, 15);
		Questions3.window.add(lversion);
	}

	public static void counterDisplay() {
		new Thread(new Runnable() {
			public void run() {
				while (Questions3.running == true) {
					try {
						Thread.sleep(1000);
					...
					if (!(QuestionTimer.getTime() == 0)) {
						if (QuestionTimer.getTime() == QuestionTimer.TotalTime) {
							Questions3.countersec.setText("Acabou!");
						} else {
							Questions3.countersec.setText("Tempo: " + QuestionTimer.getTime());
						}
					}
				}
			}
	...
	public static void drawTeams() {
		...
		size = 170;
		welcome.setBounds((Questions3.width - size) / 8, 200, size, 50);
		Questions3.window.add(welcome);

		welcome = new JLabel("Equipa 2 - " + Team2.get() + " Pontos");
		welcome.setFont(new Font("Arial", Font.BOLD, 18));
		welcome.setForeground(Color.BLUE);
		welcome.setBounds(((Questions3.width - size) * 7) / 8, 200, size, 50);
		Questions3.window.add(welcome);
	...
	public static void drawTimers() {
		...
		size = 210;
		tempo.setBounds(((Questions3.width - size) / 2) - 250, Questions3.height - 140, size, 50);
		Questions3.window.add(tempo);

		Questions3.counter = new JLabel("Tempo: 0");
		Questions3.counter.setFont(new Font("Arial", Font.BOLD, 15));
		size = 100;
		Questions3.counter.setBounds(((Questions3.width - size) / 2) - 250, Questions3.height - 120, size, 50);
		Questions3.window.add(Questions3.counter);

		tempo = new JLabel("Tempo Restante:");
		tempo.setFont(new Font("Arial", Font.BOLD, 15));
		size = 130;
		tempo.setBounds((Questions3.width - size) / 2, 120, size, 12);
		Questions3.window.add(tempo);

		Questions3.countersec = new JLabel("Tempo: " + QuestionTimer.TotalTime_Mask);
		Questions3.countersec.setFont(new Font("Arial", Font.BOLD, 15));
		size = 80;
		Questions3.countersec.setBounds((Questions3.width - size) / 2, 140, size, 12);
		Questions3.window.add(Questions3.countersec);
