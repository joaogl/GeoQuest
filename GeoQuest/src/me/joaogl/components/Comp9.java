...
import me.joaogl.game.resolution.Resolution9;
...
public class Comp9 {
	...
	public static void drawCredits() {
		...
		int size = 560;
		lversion.setBounds(((Questions9.width - size) / 2), (Questions9.height - 80), size, 15);
		Questions9.window.add(lversion);
	}

	public static void counterDisplay() {
		new Thread(new Runnable() {
			public void run() {
				while (Questions9.running == true) {
					try {
						Thread.sleep(1000);
					...
					if (!(TotalTimer.getTime() == null)) {
						if (TotalTimer.getTime() == "15:0" || TotalTimer.getTime() == "O tempo acabou.") Questions9.counter.setText("Acabou!");
						else Questions9.counter.setText("Tempo: " + TotalTimer.getTime());
					}
					if (!(QuestionTimer.getTime() == 0)) {
						if (QuestionTimer.getTime() == QuestionTimer.sec) Questions9.countersec.setText("Acabou!");
						else Questions9.countersec.setText("Tempo: " + QuestionTimer.getTime());
					}
				}
			}
		}).start();
	...
	public static void drawMenu() {
		...
		Questions9.bar.add(About);
		Questions9.bar.add(Points);
		if (ProgramInfo.debugging) Questions9.bar.add(Debugging);
	...
	public static void drawTeams() {
		...
		size = 170;
		welcome.setBounds((Questions9.width - size) / 8, 200, size, 50);
		Questions9.window.add(welcome);

		welcome = new JLabel("Equipa 2 - " + Points.getBlue() + " Pontos");
		welcome.setFont(new Font("Arial", Font.BOLD, 18));
		welcome.setForeground(Color.BLUE);
		welcome.setBounds(((Questions9.width - size) * 7) / 8, 200, size, 50);
		Questions9.window.add(welcome);
	...
	public static void drawTimers() {
		...
		size = 210;
		tempo.setBounds(((Questions9.width - size) / 2) - 250, Questions9.height - 140, size, 50);
		Questions9.window.add(tempo);

		Questions9.counter = new JLabel("Tempo: 0");
		Questions9.counter.setFont(new Font("Arial", Font.BOLD, 15));
		size = 100;
		Questions9.counter.setBounds(((Questions9.width - size) / 2) - 250, Questions9.height - 120, size, 50);
		Questions9.window.add(Questions9.counter);

		tempo = new JLabel("Tempo Restante:");
		tempo.setFont(new Font("Arial", Font.BOLD, 15));
		size = 130;
		tempo.setBounds((Questions9.width - size) / 2, 120, size, 12);
		Questions9.window.add(tempo);

		Questions9.countersec = new JLabel("Tempo: " + (QuestionTimer.sec - 1));
		Questions9.countersec.setFont(new Font("Arial", Font.BOLD, 15));
		size = 80;
		Questions9.countersec.setBounds((Questions9.width - size) / 2, 140, size, 12);
		Questions9.window.add(Questions9.countersec);

	}

	public static void close() {
		Questions9.window.removeAll();
		if (ProgramInfo.debugging) {
			if (ProgramInfo.debugquestions == Questions9.pageid) new Questions9();
			else new Resolution9();
		} else new Resolution9();
	}
}