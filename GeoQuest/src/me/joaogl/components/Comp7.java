...
import me.joaogl.game.resolution.Resolution7;
...
public class Comp7 {
	...
	public static void drawCredits() {
		...
		int size = 560;
		lversion.setBounds(((Questions7.width - size) / 2), (Questions7.height - 80), size, 15);
		Questions7.window.add(lversion);
	}

	public static void counterDisplay() {
		new Thread(new Runnable() {
			public void run() {
				while (Questions7.running == true) {
					try {
						Thread.sleep(1000);
					...
					if (!(TotalTimer.getTime() == null)) {
						if (TotalTimer.getTime() == "15:0" || TotalTimer.getTime() == "O tempo acabou.") Questions7.counter.setText("Acabou!");
						else Questions7.counter.setText("Tempo: " + TotalTimer.getTime());
					}
					if (!(QuestionTimer.getTime() == 0)) {
						if (QuestionTimer.getTime() == QuestionTimer.sec) Questions7.countersec.setText("Acabou!");
						else Questions7.countersec.setText("Tempo: " + QuestionTimer.getTime());
					}
				}
			}
		}).start();
	...
	public static void drawMenu() {
		...
		Questions7.bar.add(About);
		Questions7.bar.add(Points);
		if (ProgramInfo.debugging) Questions7.bar.add(Debugging);
	...
	public static void drawTimers() {
		...
		size = 210;
		tempo.setBounds(((Questions7.width - size) / 2) - 250, Questions7.height - 140, size, 50);
		Questions7.window.add(tempo);

		Questions7.counter = new JLabel("Tempo: 0");
		Questions7.counter.setFont(new Font("Arial", Font.BOLD, 15));
		size = 100;
		Questions7.counter.setBounds(((Questions7.width - size) / 2) - 250, Questions7.height - 120, size, 50);
		Questions7.window.add(Questions7.counter);
		...
		Questions7.window.add(Questions7.countersec);

	}

	public static void close() {
		Questions7.window.removeAll();
		if (ProgramInfo.debugging) {
			if (ProgramInfo.debugquestions == Questions7.pageid) new Questions7();
			else new Resolution7();
		} else new Resolution7();
	}
}